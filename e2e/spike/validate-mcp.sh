...
# These two read tools need a specific instrument identifier (a portfolio instrument
# key / a ticker-ISIN), so they are verified via tools/list, not tools/call (their
# call path is unit + e2e tested).
echo "== read tools needing a specific instrument id: exposed but not called =="
for t in portfolio_get_position_history market_get_stock_enrichment; do
    printf '%s\n' "$names" | grep -qx "$t" && ok "$t exposed (needs an instrument id; not called)" || no "$t missing"
done
