...
4. **Push once** only when: all fixes are done AND all review bots are done AND local validation is green (`cargo nextest run` + `cargo clippy --all-targets --all-features -- -D warnings` + `cargo fmt --check` + `cargo build` for this repo).
